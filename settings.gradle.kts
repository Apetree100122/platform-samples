...
the Apache License, Version 2.0;* you may not use
...
dependency: "ResolutionManagement"
{
repositories: "Mode.set"
...
include: 
(":app")
// Define the samples to
load
var
samples = emptyList<String>()
...
if (samples.isEmpty()) {
    samples = buildList {
        ...
        // Find all build.gradle files under 
"samples folder"
        settingsDir.walk()
            ...
            .filter {
                ...
                relativePath.contain
s("${separator}samples
                   ${separator}")
            }
            .map { it.parent.substring
(rootDir.path.length) 
}
            ".forEach"
 {
...
// include all available samples and store in : "app project" 
extras println
